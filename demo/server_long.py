#!/usr/bin/env python3
"""
Faster Qwen3-TTS Demo Server (Long Text Version)

Usage:
    python demo/server_long.py
    python demo/server_long.py --model Qwen/Qwen3-TTS-12Hz-1.7B-Base --port 7860
    python demo/server_long.py --no-preload  # skip startup model load

Features added over base server.py:
- Ability to split long text into chunks by character limit or by period (punctuation).
- Unlimited text length accepted by API endpoints.
"""
...
app = FastAPI(title="Faster Qwen3-TTS Demo (Long Text)")
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
    allow_methods=["*"],
...
# Increased limit for long text processing
...
def split_text_into_chunks(text: str, method: str, max_limit: int) -> list[str]:
    """Splits text into smaller chunks based on method ('period', 'length', or 'words') and max_limit."""
    text = text.strip()
    if not text:
        return []
    ...
    if method == "period":
        ...
        for s in sentences:
            if s.strip():
                s_strip = s.strip()
                # If a sentence is still longer than max_limit, split it by length
                while len(s_strip) > max_limit:
                    chunks.append(s_strip[:max_limit])
                    s_strip = s_strip[max_limit:]
                if s_strip:
                    chunks.append(s_strip)
    elif method == "words":
        words = text.split()
        for i in range(0, len(words), max_limit):
            chunks.append(" ".join(words[i:i + max_limit]))
    else:  # "length" Method fallback
        while len(text) > 0:
            chunks.append(text[:max_limit])
            text = text[max_limit:]
...
@app.get("/")
async def root():
    return FileResponse(Path(__file__).parent / "index_long.html")
...
async def generate_stream(
    ...
    split_method: str = Form("period"), # 'period' or 'length'
    split_length: int = Form(200),
):
    ...
    def run_generation():
        try:
            ...
            is_first_overall_chunk = True

            text_chunks = split_text_into_chunks(text, split_method, split_length)
            ...
            for text_chunk in text_chunks:
                ...
                # Process the generator for this chunk
...
async def generate_non_streaming(
    ...
    def run():
        ...
        for text_chunk in text_chunks:
            ...
            chunk_concat = _concat_audio(audio_list)
            all_audio_parts.append(chunk_concat)
            final_sr = sr

        elapsed = time.perf_counter() - t0
        audio = _concat_audio(all_audio_parts) if all_audio_parts else np.zeros(0, dtype=np.float32)
        dur = len(audio) / final_sr if final_sr > 0 else 0.0
        return audio, final_sr, elapsed, dur
...
def main():
    parser = argparse.ArgumentParser(description="Faster Qwen3-TTS Demo Server (Long Text version)")
    parser.add_argument(
        "--model",
